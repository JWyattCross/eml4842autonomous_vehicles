...
from gps_nav.uf_support.route_support import get_rad_of_curvature_to_carrot, determine_arc_radius, get_cross_track_and_heading_error
...
class VehicleController(Node):
    def __init__(self):
        ...
        self.errorTxt = open(('error' + str(datetime.datetime.now()) + '.txt'), 'w')
        self.errorTxt.write(f'Error\n')
    ...
    def save_crosstrack_error(self):
        ...
        self.e = np.linalg.norm(self.closest_point, self.vehicle_point)
        self.errorTxt.write(str(self.e)+',')
    ...
    def main_timer_callback(self):
        ...
        if self.have_goal_pose and self.have_vehicle_pose:
            ...
            # arc_radius = determine_arc_radius(
            #     self.vehicle_point,
            #     self.vehicle_heading_rad,
            #     self.current_goal_point)
            
            # out_msg.drive.speed = self.speed
            # angle_rad = math.atan(self.L_wheelbase_m / arc_radius)
            # out_msg.drive.steering_angle = angle_rad

            # Stanley Controller

            crosstrack_error, heading_error, line_point = get_cross_track_and_heading_error(
                self.closest_point, 
            ...
            out_msg.drive.speed = self.speed
            angle_rad = heading_error + math.atan(K * crosstrack_error / self.speed)

            maximum = 0.8
            if angle_rad > maximum:
                angle_rad = maximum
            elif angle_rad < -maximum:
                angle_rad = -maximum
            ...
            # desired = -2*np.pi + np.arctan2(self.current_goal_point[1]-self.vehicle_point[1], self.current_goal_point[0]-self.vehicle_point[0])

            # et = desired - self.vehicle_heading_rad

            # self.a0 =self.Kp*(et+self.et1) + self.Ki*et + self.Kd*(et- 2*self.et1 + self.et2)

            # out_msg.drive.steering_angle = self.a0
            # out_msg.drive.speed = 2.0

            # self.et2 = self.et1
            # self.et1 = self.et
            # self.a1 = self.a0

            self.errorTxt.write(f'{crosstrack_error}\n')
            self.publisher.publish(out_msg)
...
def main(args=None):
    ...
    #vehicle_controller.errorTxt.close()
